...
/* inline list manipulation real time procedures */

#define	rtisempty(list)	(rtq[(list)].qnext >= NPROC)
#define	rtnonempty(list)	(rtq[(list)].qnext < NPROC)
#define	rtfirstkey(list)	(rtq[rtq[(list)].qnext].qkey)
#define rtlastkey(tail)	(rtq[rtq[(tail)].qprev].qkey)
#define rtfirstid(list)	(rtq[(list)].qnext)
