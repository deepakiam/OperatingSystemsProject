...
/*    count=0;
    ...
    resume(prB = create(procsleep, 2000, 100000000, "proc B", 1, 'B'));
    while (count++ < LOOP) {
        kprintf("M");
...
*/
    ...
    resume(prA);
	kprintf("created real time process B\n");
    resume(prB);
    sleep(5);
    kill(prA);
    kill(prB);
    ...
    c_percent=0;
    a_cnt=b_cnt=c_cnt=0;
    kprintf("creating process B\n");
    prB = createReal(proc_b, 2000, 10, "proc B", 1, 'B');
	kprintf("creating process C\n");
    ...
    kill(prB);
    kill(prC);
    total_cnt = b_cnt + c_cnt;
    
    b_percent = (double) b_cnt / total_cnt * 100;
    c_percent = (double) c_cnt / total_cnt * 100;
    kprintf("Test RESULT: B = %d, C = %d (%d : %d)\n", b_cnt, c_cnt, (int) b_percent, (int) c_percent);
